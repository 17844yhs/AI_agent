...
| 概念                    | 说明                   | 特点                                                                                        |
| ----------------------- | ---------------------- | ------------------------------------------------------------------------------------------- |
| **State（状态）** | Graph 中共享的数据容器 | • 使用`TypedDict` 定义• 节点可以读取 State• 节点可以更新 State                         |
| **Node（节点）**  | 处理逻辑单元           | • 接收当前 State• 执行处理逻辑• 返回更新的 State                                         |
| **Edge（边）**    | 连接节点的路径         | •**普通边**：A → B（无条件）• **条件边**：A → [判断] → B/C（根据条件选择） |
